...
// --- 静态配置 ---
const TWITTER_DOMAINS = ["twimg.com", "twitter.com", "x.com", "t.co","cloudflare-dns.com","pages.dev","workers.dev","cloudflare.com","lss1.ccwu.cc"]; //您可以添加某域名强制解析到CF，填写 x.com 时包含 *.x.com，适用于仅ipv4访问或多CDN负载均衡的站点
...
async function handleDnsQuery(rawBuffer, config, ctx) {
    try {
        ...
        if (isTwitter) {
            ...
            if (qType === 1) {
                let replaceIps = null;
                if (config.ip4) replaceIps = config.ip4.split(',').map(ipToBytes);
                else if (config.cfDomain) replaceIps = await fetchReplacementIps(config.cfDomain, 1, ctx);
                else replaceIps = [ipToBytes(DEFAULT_TWITTER_IP)];
        ...
        if (qType === 1 || qType === 28) {
            ...
            if (ownerData === 'CF') {
                let replaceIps = null;
                if (qType === 1 && config.ip4) replaceIps = config.ip4.split(',').map(ipToBytes);
                else if (qType === 28 && config.ip6) replaceIps = config.ip6.split(',').map(ipv6ToBytes);
...
async function getOwnerFromCache(name) {
    if (GLOBAL_CACHE.has(name)) {
        ...
        GLOBAL_CACHE.delete(name);
    }
    try {
        if (typeof caches !== 'undefined' && caches.default) {
            const cachedRes = await caches.default.match(new Request(`https://worker-cache/owner/${name}`));
            if (cachedRes) {
                const owner = await cachedRes.text();
                GLOBAL_CACHE.set(name, { val: owner, expire: Date.now() + MEM_CACHE_TTL });
                return owner;
            }
        }
    } catch (e) {}
...
function setOwnerCache(name, owner, ctx) {
    GLOBAL_CACHE.set(name, { val: owner, expire: Date.now() + MEM_CACHE_TTL });
    try {
        if (ctx && typeof caches !== 'undefined' && caches.default && owner) {
            const r = new Response(owner, { headers: { 'Cache-Control': 'max-age=3600' } });
            ctx.waitUntil(caches.default.put(new Request(`https://worker-cache/owner/${name}`), r));
        }
    } catch (e) {}
}
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.Answer) {
                ...
                for (const ip of ips) {
                    if (isIpInCompiledCidrs(ip, COMPILED_META)) { detectedOwner = 'META'; break; }
                    if (isIpInCompiledCidrs(ip, COMPILED_CF)) { detectedOwner = 'CF'; break; }
                }
                if (detectedOwner) {
...
async function fetchCleanEchRdata(domain, ctx) {
    let data = null;
    const cacheUrl = `https://worker-cache/ech/${domain}`;
    try {
        if (typeof caches !== 'undefined' && caches.default) {
            const cachedRes = await caches.default.match(new Request(cacheUrl));
            if (cachedRes) data = await cachedRes.json();
        }
    } catch (e) { }

    if (!data) {
        try {
            const res = await fetch(`${UPSTREAM_JSON}?name=${domain}&type=65`, { headers: { 'Accept': 'application/dns-json' } });
            if (res.ok) {
                data = await res.json();
                try {
                    if (ctx && typeof caches !== 'undefined' && caches.default && data.Status === 0) {
                        const r = new Response(JSON.stringify(data), {
                            headers: { 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=600' }
                        });
                        ctx.waitUntil(caches.default.put(new Request(cacheUrl), r));
                    }
                } catch (e) {}
            }
        } catch (e) { return null; }
    ...
    if (data && data.Status === 0 && data.Answer) {
        try {
            ...
            return packHttpsParams(parseInt(parts[0]) || 0, parts[1], params);
        } catch (e) { return null; }
    }
    return null;
}
...
async function fetchReplacementIps(domain, type, ctx) {
    const cacheType = type === 1 ? 'ip4' : 'ip6';
    const cacheKeyUrl = `https://worker-cache/resolve/${domain}/${cacheType}`;
    try {
        if (typeof caches !== 'undefined' && caches.default) {
            const cachedRes = await caches.default.match(new Request(cacheKeyUrl));
            if (cachedRes) {
                const ips = await cachedRes.json();
                return ips.map(ip => type === 1 ? ipToBytes(ip) : ipv6ToBytes(ip));
            }
        }
    } catch (e) {}
    try {
        const res = await fetch(`${UPSTREAM_JSON}?name=${domain}&type=${type}`, { headers: { 'Accept': 'application/dns-json' } });
        if (res.ok) {
            const data = await res.json();
            if (data.Answer) {
                const ips = data.Answer.filter(a => a.type === type).map(a => a.data);
                if (ips.length > 0) {
                    if (ctx && typeof caches !== 'undefined' && caches.default) {
                        const r = new Response(JSON.stringify(ips), {
                            headers: { 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=600' }
                        });
                        ctx.waitUntil(caches.default.put(new Request(cacheKeyUrl), r));
                    }
                    return ips.map(ip => type === 1 ? ipToBytes(ip) : ipv6ToBytes(ip));
                }
            }
...
function isIpInCompiledCidrs(ip, compiledRanges) {
    if (ip.includes(':')) {
        try {
            const ipBn = ipv6ToBigInt(ip);
            for (const range of compiledRanges.v6) if (ipBn >= range.start && ipBn <= range.end) return true;
        } catch(e) {}
    } else {
        try {
            const ipNum = ipToLong(ip);
            for (const range of compiledRanges.v4) if (ipNum >= range.start && ipNum <= range.end) return true;
        } catch(e) {}
    }
    return false;
}
...
function packHttpsParams(priority, target, params) {
    ...
    paramBufs.forEach(b => { res.set(b, offset); offset += b.length; });
    return res;
}
...
function encodeSvcParam(key, value) {
    ...
    v.setUint16(0, id); v.setUint16(2, valBuf.length); res.set(valBuf, 4);
    return res;
}

function encodeDnsName(domain) {
    const parts = domain.split('.'); const buf = new Uint8Array(domain.length + 2); let offset = 0;
    for (const part of parts) { buf[offset++] = part.length; for (let i = 0; i < part.length; i++) buf[offset++] = part.charCodeAt(i); }
...
function parseDnsPacket(buf) {
    ...
    return { id: v.getUint16(0), questions: [{ name: labels.join('.'), type: v.getUint16(offset) }] };
}

function extractIpsFromPacket(buffer) {
    const ips = []; const view = new DataView(buffer); if (buffer.byteLength < 12) return [];
    const ancount = view.getUint16(6); const totalRecords = ancount + view.getUint16(8) + view.getUint16(10);
    let offset = 12;
    try {
...
function ipv6ToBytes(ip) {
    let p = ip.split(':');
    if (ip.includes('::')) {
        const [l, r] = ip.split('::'), lp = l ? l.split(':') : [], rp = r ? r.split(':') : [];
        p = [...lp, ...Array(8 - lp.length - rp.length).fill('0'), ...rp];
    }
    const b = new Uint8Array(16);
    p.forEach((v, i) => { const val = parseInt(v, 16) || 0; b[i * 2] = val >> 8; b[i * 2 + 1] = val & 0xFF; }); return b;
}
